...
# CUDA_VERSION, dang "MAJOR.MINOR" (vd "12.8", "12.9", "13.0"; patch nhu
# "12.8.1" cung duoc, se bi bo qua) hoac dang nen "MAJORMINOR" (vd "128").
# Tu bien nay script suy ra apt package, duong dan lib, torch index va dung
# dung wheel vllm chinh thuc (vllm+cuXXX) thay vi de pip tu chon ban mac dinh
# (co the doi theo thoi gian).
#
# 3 kich ban dung:
#   A. Dev tren Colab (mac dinh):            bash setup_vllm.sh
#   B. Nop bai cho BTC (CUDA da co san):      SKIP_CUDA=1 CUDA_VERSION=12.8 bash setup_vllm.sh
#   C. Test tren Colab gia lap CUDA cua BTC:  CUDA_VERSION=12.8 bash setup_vllm.sh
...
CUDA_VERSION="${CUDA_VERSION:-13.0}"         # chap nhan "12.8", "12.8.1", hoac dang nen "128"
...
# Chuan hoa CUDA_VERSION ve major/minor, du nguoi dung truyen "12.8", "12.8.1"
# (bo qua patch) hay dang nen "128".
...
if [[ -z "$CUDA_MAJOR" || -z "$CUDA_MINOR" || ! "$CUDA_MAJOR" =~ ^[0-9]+$ || ! "$CUDA_MINOR" =~ ^[0-9]+$ ]]; then
  echo "ERROR: CUDA_VERSION='${CUDA_VERSION}' khong hop le. Dung dang 'MAJOR.MINOR' (vd '12.8') hoac nen 'MAJORMINOR' (vd '128')." >&2
  exit 1
fi
...
VLLM_WHEEL_URL="${VLLM_WHEEL_URL:-https://github.com/vllm-project/vllm/releases/download/v${VLLM_VERSION}/vllm-${VLLM_VERSION}+cu${CUDA_COMPACT}-cp38-abi3-manylinux_2_35_${CPU_ARCH}.whl}"
...
if [ "$SKIP_PYTHON_DEPS" != "1" ]; then
  echo "========================================"
  echo "2. DANG DONG BO TORCH VA VLLM ${VLLM_VERSION} (cu${CUDA_COMPACT})..."
  echo "========================================"
